"""DeepSeek API provider — uses stdlib only, no third-party HTTP client."""
...
class DeepSeekProvider(ModelProvider):
    ...
    * ``DEEPSEEK_TIMEOUT`` (default 30 seconds)

    If ``DEEPSEEK_API_KEY`` is not set, ``generate()`` raises a
    clear RuntimeError — the constructor itself does not fail so
    that the provider can be imported and inspected without a key.
    """
    ...
    def generate(
        ...
        *,
        ...
        try:
            ...
            raise RuntimeError(
                f"DeepSeek API 返回格式异常: {json.dumps(data, ensure_ascii=False)[:500]}"
            ) from exc
